...
description: A股申万二级行业(124个)板块行情分析，纯 Python + akshare 实现，不依赖任何 MCP 连接器。按周涨幅/月涨幅双维度排行，生成图表与上涨原因分析，输出单文件 HTML 报告。只做涨幅榜，不出跌幅榜。触发场景：板块行情、行业涨幅、板块排名、哪些板块涨了、行业轮动、板块分析、sector performance、申万二级。
agent_created: true
---

# A股板块行情分析（akshare 独立版）

对申万二级行业（124 个）做多周期**涨幅**分析，输出双维度涨幅排行、可视化图表与上涨原因解读，最终产出一份**自包含单文件 HTML 报告**。
...
按 `references/report-template.md`：数据说明 → 周涨幅 Top 30 表格 + 要点解读 + Top10 上涨原因 → 月涨幅 Top 30 表格 + 要点解读 + Top10 上涨原因 → 三条主线 + 风险提示 + 免责声明。
